...
st.set_page_config(
    layout="wide",
    page_title="Red San Pablo - Adulto Mayor",
    page_icon="LOGO.png",
...
with st.sidebar:
    ...
    <div class="filtros-header">
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg>
      Filtros
    </div>
    ''', unsafe_allow_html=True)
    ...
    # Leer DNI anticipadamente para auto-detectar IPRESS
    if "dni_input" not in st.session_state:
        st.session_state["dni_input"] = ""

    _dni_preview = st.session_state.get("dni_input", "")
    lista_dni_sidebar = re.findall(r'\d+', _dni_preview) if _dni_preview else []
    if lista_dni_sidebar:
        df_dni_lookup = df_tmp.filter(
            pl.col("Numero_Documento_Paciente").cast(pl.Utf8).str.strip_chars().is_in(lista_dni_sidebar)
        )
        ipress_de_dni = sorted([str(i).strip() for i in df_dni_lookup["Nombre_Establecimiento"].unique().to_list()])
        default_sel = ipress_de_dni if ipress_de_dni else [i for i in lista_ipress if target_name in i]
    else:
        default_sel = [i for i in lista_ipress if target_name in i]

    st.markdown('''
    <div class="sidebar-label">
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#475569" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
      IPRESS
    </div>
    ''', unsafe_allow_html=True)
    sel_ipress = st.multiselect("IPRESS", options=lista_ipress, default=default_sel, label_visibility="collapsed")

    st.markdown('<div class="sidebar-divider"></div>', unsafe_allow_html=True)

    st.markdown('''
    <div class="sidebar-label">
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#475569" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
      Mes de Atención
    </div>
    ''', unsafe_allow_html=True)
    df_meses_lista = df_tmp.select(["Mes_Num", "Mes_Nombre"]).unique().sort("Mes_Num")
    sel_mes = st.multiselect("Mes de Atención", options=df_meses_lista["Mes_Nombre"].to_list(), label_visibility="collapsed")

    st.markdown('<div class="sidebar-divider"></div>', unsafe_allow_html=True)

    st.markdown('''
    <div class="sidebar-label">
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#475569" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
      Buscar DNI(s)
    </div>
    ''', unsafe_allow_html=True)
    ...
    <div class="sidebar-footer">
      <p style="font-size:0.65rem;color:#cbd5e1;letter-spacing:0.06em;font-weight:500;margin:0;">
        RED SAN PABLO · ADULTO MAYOR<br>
        <span style="color:#e2e8f0;">© César Malca — 2026</span>
      </p>
    </div>
    ''', unsafe_allow_html=True)
...
<div class="hero-header">
    ...
    <p class="hero-sub">Población objetivo: 60 años a mas· 31 indicadores evaluados</p>
</div>
...
df_base = df_base.filter(
    (pl.col("Anio_Actual_Paciente") >= 60) & (pl.col("Anio_Actual_Paciente") <= 200)
)

lista_dni = re.findall(r'\d+', sel_dni_raw) if sel_dni_raw else []

if lista_dni:
    # Si hay DNIs: filtrar solo por DNI (IPRESS ya fue auto-detectado en sidebar,
    # pero aplicamos el filtro IPRESS del multiselect que el usuario puede ver/editar)
    df_base = df_base.filter(
    ...
    # Sin DNIs: aplicar filtros normales de IPRESS y mes
    if sel_ipress:
...
if not df_base.is_empty():
    ...
    # fill_null("") evita que un campo vacío convierta toda la celda en null
    df_base = df_base.with_columns([
        (pl.col("Apellido_Paterno_Personal").fill_null("") + " " +
         pl.col("Apellido_Materno_Personal").fill_null("") + " " +
         pl.col("Nombres_Personal").fill_null(""))
        .str.replace_all(r"\s{2,}", " ").str.strip_chars().alias("Profesional"),
        (pl.col("Apellido_Paterno_Paciente").fill_null("") + " " +
         pl.col("Apellido_Materno_Paciente").fill_null("") + " " +
         pl.col("Nombres_Paciente").fill_null(""))
        .str.replace_all(r"\s{2,}", " ").str.strip_chars().alias("Paciente"),
        pl.col("Anio_Actual_Paciente").cast(pl.Int64).alias("Anio_Actual_Paciente"),
    ])
   ...
   # ── FUNCIÓN AUXILIAR: Obtener la celda de fecha+valor para cada ítem ──────
    def extraer_item(df: pd.DataFrame, cod_item: str, id_col: str) -> dict:
        ...
        # ── CASO ESPECIAL: columna combinada 86703.01 OR 86318.01 ──────────────
        if id_col == "__VIH_PR__":
            resultado = {}
            for dni, grupo in df.groupby("Numero_Documento_Paciente"):
                tiene_86703 = not grupo[grupo["Codigo_Item"] == "86703.01"].empty
                tiene_86318 = not grupo[grupo["Codigo_Item"] == "86318.01"].empty

                if tiene_86703 and tiene_86318:
                    resultado[str(dni)] = "⚠️ Error: No pueden ir Juntos"
                elif tiene_86703:
                    fila = grupo[grupo["Codigo_Item"] == "86703.01"].sort_values("Fecha_Atencion", ascending=False).iloc[0]
                    ...
                    fila = grupo[grupo["Codigo_Item"] == "86318.01"].sort_values("Fecha_Atencion", ascending=False).iloc[0]
                    fecha_str = fila["Fecha_Atencion"].strftime("%d/%m/%Y")
                    vlab = str(fila["Valor_Lab"]).strip()
                    resultado[str(dni)] = f"{fecha_str} ({vlab})" if vlab and vlab not in ["", "nan", "None"] else fecha_str
                # si no tiene ninguno → no se agrega (quedará ❌)
            return resultado
        ...
        elif id_col == "99402.08_1":
            mask = (df["Codigo_Item"] == cod_upper) & (df["Valor_Lab"] == "1")
        elif id_col == "99402.08_2":
            mask = (df["Codigo_Item"] == cod_upper) & (df["Valor_Lab"] == "2")
        else:
    ...
    # Las celdas de error (⚠️) no cuentan como realizadas
    ...
    with col1:
        ...
        <div class="metric-card">
            <div class="metric-icon"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#1C398E" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="7" r="3"/><circle cx="16" cy="8" r="2.5"/><path d="M2 20c0-3.3 3.1-6 7-6s7 2.7 7 6"/><path d="M20 20c0-2.2-1.8-4-4-4"/></svg></div>
    ...
    with col2:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-icon"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#059669" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg></div>
    ...
    with col3:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-icon"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#b45309" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/></svg></div>
    ...
    with col4:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-icon"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#dc2626" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M12 9v4m0 4h.01M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/></svg></div>
            <div class="metric-label">Críticos &lt;50%</div>
            <div class="metric-value red">{criticos}</div>
        </div>""", unsafe_allow_html=True)
    with col5:
        color_prom = "#059669" if avance_prom >= 70 else "#b45309" if avance_prom >= 40 else "#dc2626"
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-icon"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="{color_prom}" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M18 20V10"/><path d="M12 20V4"/><path d="M6 20v-6"/></svg></div>
    ...
    def _estilo_fila(row):
        """Colorea toda la fila según el avance del paciente."""
        avance = row.get("Avance %", 0)
        if avance >= 99.9:
            bg = "background-color: #f0fdf4"   # verde muy suave
        elif avance >= 50:
            bg = "background-color: #fefce8"   # amarillo muy suave
        else:
            bg = "background-color: #fff1f2"   # rojo muy suave
        return [bg] * len(row)

    # ── TABLA HTML (modelo app.py) ───────────────────────────────────────────
    df_display = df_final.rename(columns={"Numero_Documento_Paciente": "DNI"})

    # DNI y Paciente primero, resto de columnas después
    sticky_cols = ["DNI", "Paciente"]
    rest_cols   = [c for c in df_display.columns if c not in sticky_cols]
    cols_all    = sticky_cols + rest_cols
    df_display  = df_display[cols_all]

    # Anchos acumulados para sticky: # = 40px, DNI = 90px, Paciente = auto
    ths = (
        '<th class="jt-sticky-idx">#</th>'
        '<th class="jt-sticky-dni">DNI</th>'
        '<th class="jt-sticky-pac">Paciente</th>'
        + "".join(f"<th>{c}</th>" for c in rest_cols)
    )

    rows_html = ""
    for i, (_, row) in enumerate(df_display.iterrows(), start=1):
        avance = row.get("Avance %", 0)
        try: avance = float(avance)
        except: avance = 0
        row_bg = "jt-bgok" if avance >= 99.9 else ("jt-bgmd" if avance >= 50 else "jt-bgno")

        # Fondo explícito para celdas sticky (no depender de inherit)
        bg_color = "#f0fdf4" if avance >= 99.9 else ("#fefce8" if avance >= 50 else "#fff1f2")
        sticky_bg = f"background:{bg_color};"
        celdas = (
            f'<td class="jt-idx jt-sticky-idx" style="{sticky_bg}">{i}</td>'
            f'<td class="jt-sticky-dni" style="{sticky_bg}">{row["DNI"]}</td>'
            f'<td class="jt-sticky-pac" style="{sticky_bg}">{row["Paciente"]}</td>'
        )
        for col in rest_cols:
            val = row[col]
            disp = f"{val:.1f}" if col == "Avance %" else ("" if (isinstance(val, float) and val != val) else str(val))
            if col in columnas_indicadores:
                if disp == "❌":            css = "jt-no"
                elif disp.startswith("⚠️"): css = "jt-warn"
                else:                       css = "jt-ok"
            elif col == "Avance %":
                try:
                    v = float(val)
                    css = "jt-avok" if v >= 99.9 else ("jt-avmd" if v >= 50 else "jt-avno")
                except: css = ""
            elif col == "Realizados": css = "jt-num"
            elif col == "Faltan":     css = "jt-grey"
            else:                     css = row_bg
   ...
   # ── DESCARGA ──────────────────────────────────────────────────────────────
    ...
    with col_dl:
        ...
        with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
            df_final.to_excel(writer, index=True, sheet_name="Adulto Mayor")
        excel_bytes = buffer.getvalue()
        st.download_button(
            label="Descargar Excel",
            data=excel_bytes,
            file_name="adulto_mayor_san_pablo_2026.xlsx",
    ...
    st.markdown("""
    <div class="no-data">
        <div class="no-data-icon"><svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#cbd5e1" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4.35-4.35"/></svg></div>
