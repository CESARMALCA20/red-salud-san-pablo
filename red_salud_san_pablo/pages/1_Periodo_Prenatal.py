...
st.set_page_config(
    layout="wide",
    page_title="Red San Pablo - Prenatal",
    page_icon="LOGO.png",
...
<div class="topbar-sp">
    <div class="topbar-sp-left">
        ...
        <div>
            ...
            <div class="topbar-sp-sub">Curso de Vida &nbsp;·&nbsp; Período Prenatal</div>
        </div>
    </div>
...
with st.sidebar:
    ...
    st.markdown(
        ...
        'RED SAN PABLO · PRENATAL<br>© César Malca - 2026</p>',
        unsafe_allow_html=True
    )
...
<div class="hero-header">
    ...
    <h1 class="hero-title">SEGUIMIENTO DE PAQUETE SEGUIMIENTO DE PAQUETE <span>PERÍODO PRENATAL</span></h1>
    <p class="hero-sub">Población objetivo: Gestantes · 30 indicadores evaluados</p>
</div>
...
df_base = df_raw.with_columns([
    ...
    # Limpieza estricta de códigos: strip + uppercase
...
# Filtro edad adulto
# ─── FILTRO POR CÓDIGOS DE GESTANTES (PRENATAL) ──────────────────────────────
# Lista de códigos para identificar gestantes (Curso de Vida: Prenatal)
codigos_gestantes = ["Z3491", "Z3591", "Z3492", "Z3592", "Z3493", "Z3593", "Z349", "Z359"]

# Aplicar filtro por 'Codigo_Item' (asegurando limpieza de espacios)
df_base = df_base.filter(
    pl.col("Codigo_Item").str.strip_chars().is_in(codigos_gestantes)
)

# ─── CREAR df_tmp PARA LOS FILTROS DEL SIDEBAR (DESPUÉS DEL FILTRO DE GESTANTES) ───
# IMPORTANTE: df_tmp debe usar los mismos datos filtrados
df_tmp = df_base.select([
    "Fecha_Atencion", "Mes_Num", "Mes_Nombre", "Nombre_Establecimiento", "Numero_Documento_Paciente"
]).unique()
...
if sel_mes:
    df_base = df_base.filter(
        pl.col("Mes_Nombre").is_in(sel_mes)
    )
 ...
 # ── DESCARGA ──────────────────────────────────────────────────────────────
    ...
    with col_dl:
        ...
        with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
            df_final.to_excel(writer, index=True, sheet_name="Período Prenatal")
        excel_bytes = buffer.getvalue()
        st.download_button(
            label="📥 Descargar Reporte Excel",
            data=excel_bytes,
            file_name="prenatal_san_pablo_2026.xlsx",
