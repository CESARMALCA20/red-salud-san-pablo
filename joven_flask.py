...
joven_bp = Blueprint("joven", __name__)

_dir = os.path.dirname(os.path.abspath(__file__))
PARQUET_PATH = os.path.join(_dir, "data", "reporte.parquet")

ITEMS_CONFIG = [
    ("99801",    "99801_1",    "99801 PLAN ELABORADO"),
    ("Z019",     "Z019",       "Z019 VALORACION RIESGO"),
    ("99401.13", "99401.13",   "99401.13 ESTILOS VIDA"),
    ("99199.22", "99199.22",   "99199.22 PRESION ARTERIAL"),
    ("99209.04", "99209.04",   "99209.04 EVAL. ANTROPOMETRICA"),
    ("99403.01", "99403.01",   "99403.01 CONS. NUTRICIONAL"),
    ("99402.03", "99402.03",   "99402.03 CONS. SSYRR"),
    ("96150.01", "96150.01",   "96150.01 VIF"),
    ("96150.02", "96150.02",   "96150.02 ALCOHOL Y DROGAS"),
    ("96150.03", "96150.03",   "96150.03 DEPRESION PHQ-9"),
    ("99402.09", "99402.09",   "99402.09 CONS. SALUD MENTAL"),
    ("D0150",    "D0150",      "D0150 ORAL"),
    ("99401.33", "99401.33",   "99401.33 PRE-TEST VIH"),
    ("__VIH_PR__","__VIH_PR__","86703/86318 DETECCION VIH"),
    ("99401.34", "99401.34",   "99401.34 POST-TEST VIH"),
    ("99402.08", "99402.08_1", "99402.08 CONS. CANCER (1)"),
    ("99402.08", "99402.08_2", "99402.08 CONS. CANCER (2)"),
    ("A150",     "A150",       "A150 TUBERCULOSIS"),
    ("99199.58", "99199.58",   "99199.58 MUESTRA TBC"),
    ("99401",    "99401",      "99401 CONS. INTEGRAL"),
    ("99801",    "99801_TA",   "99801 PLAN EJECUTADO"),
]
    ...
    if id_col == "__VIH_PR__":
        ...
        for dni, grupo in df.groupby("Numero_Documento_Paciente"):
            ...
            if tiene_86703 and tiene_86318:
                resultado[str(dni)] = "⚠ Error: No pueden ir Juntos"
            elif tiene_86703:
...
def procesar_datos(ipress_sel, mes_sel, dni_raw):
    ...
    df = df.filter((pl.col("Anio_Actual_Paciente") >= 18) & (pl.col("Anio_Actual_Paciente") <= 29))
...
<title>Red San Pablo — Joven</title>
...
<div class="topbar">
  <div class="topbar-left">
    ...
    <div>
      ...
      <div class="topbar-sub">Curso de Vida · Joven</div>
    </div>
  </div>
...
<div class="content">
  <!-- HERO -->
  <div class="hero">
    <h1>SEGUIMIENTO DE PAQUETE <span>JOVEN</span></h1>
    <p>Población objetivo: 18 – 29 años · {N_ITEMS} indicadores evaluados</p>
  </div>

  <!-- FILTERS -->
  <form method="GET" action="/joven">
    <div class="filters">
...
function descargarExcel() {{
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Joven');

    var fecha = new Date();
    var nombreArchivo = 'AdultoMayor_' + fecha.getFullYear() + ('0'+(fecha.getMonth()+1)).slice(-2) + ('0'+fecha.getDate()).slice(-2) + '.xlsx';
